...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        alpha_val = (self.feat.zscore(((self.feat.rsi(high, timeperiod=14) - self.feat.ema(self.feat.morning_star(open_, high, low, close), timeperiod=5)) / (self.feat.adx(open_, self.feat.doji(open_, high, low, close), self.feat.morning_star(open_, high, low, close), timeperiod=60) + 1e-8)), timeperiod=10) > 1.0)
        ...
        raw_pos = self.op.where(z_score > 1.0, 0.3, self.op.where(z_score < -1.0, -0.3, 0.0))
