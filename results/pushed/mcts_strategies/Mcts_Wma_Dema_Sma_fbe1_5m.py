...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        alpha_val = self.feat.wma(self.feat.dema(self.feat.sma(self.feat.atr(high, low, close, timeperiod=30), timeperiod=10), timeperiod=10), timeperiod=60)
        ...
        raw_pos = self.op.where(scaled > 0.5, 0.1, self.op.where(scaled < -0.5, -0.1, 0.0))
        
        flat_mask = raw_pos == 0.0
        long_mask = raw_pos == 0.1
        short_mask = raw_pos == -0.1
