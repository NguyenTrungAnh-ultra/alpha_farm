...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        alpha_val = (self.feat.zscore(((self.feat.ema(self.feat.hammer(open_, high, low, close), timeperiod=60) * self.feat.var(open_, timeperiod=60)) / (self.feat.stddev(open_, timeperiod=60) + 1e-8)), timeperiod=20) > 1.0)
        ...
        raw_pos = self.op.where(z_score > 1.0, 0.35, self.op.where(z_score < -1.0, -0.35, 0.0))
