...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        alpha_val = (self.feat.zscore((self.op.diff(self.feat.macd(low)[0], periods=1) / (self.feat.vwap(high, low, close, volume) + 1e-8)), timeperiod=10) > 2.0)
        ...
        raw_pos = self.op.where(z_score > 2.5, 0.2, self.op.where(z_score < -2.5, -0.2, 0.0))
