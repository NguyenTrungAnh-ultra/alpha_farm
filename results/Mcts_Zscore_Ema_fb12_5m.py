...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        alpha_val = (self.feat.zscore(self.feat.zscore((self.feat.ema(low, timeperiod=20) - self.feat.macd(open_)[0]), timeperiod=20), timeperiod=60) > 0.5)
        ...
        raw_pos = self.op.where(z_score > 0.5, 0.25, self.op.where(z_score < -0.5, -0.25, 0.0))
