...
class DeepSeekAPI:
    ...
    def _parse_chunk(self, chunk: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
                ...
                if 'v' in data:
                    ...
                    if 'p' in data:
                        ...
                        if p == 'response/thinking_content':
                            self._current_stream_mode = 'thinking'
                        elif p == 'response/content':
                            self._current_stream_mode = 'text'
                        elif p == 'response/status' and v == 'FINISHED':
                            return {'content': '', 'type': 'text', 'finish_reason': 'stop'}
                    
                    if isinstance(v, str):
                        # Ensure stream mode is set
                        mode = getattr(self, '_current_stream_mode', 'text')
                        return {
                            'content': v,
                            'type': mode,
                            'finish_reason': None
            ...
            pass # Ignore broken json chunks
        except Exception as e:
            pass # Ignore other parse errors
