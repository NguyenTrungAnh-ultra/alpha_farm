...
graph TD
    ...
    subgraph LLM_Engine ["Phase 1: LLM Engine"]
        run_all -->|1. Gọi LLM sinh ý tưởng JSON| pipeline(agent/pipeline.py)
        pipeline --> convert(agent/convert_ideas.py)
    end
    ...
    subgraph Local_Environment ["Môi trường Local (Sandbox)"]
        direction TB
        convert -->|Validate AST Sandbox| xno_emu[xno_sdk/emulator.py]
...
- **Săn Alpha bằng MCTS (Không cần AI):** Tự động dò tìm công thức toán học và lưu file.
  ...
  ```
- **Sinh Ý Tưởng bằng LLM (Deepseek/Gemini):** Dùng AI để viết kịch bản giao dịch ra file JSON.
  ```bash
  python agent/pipeline.py
  ```
- **Trình Biên Dịch Ý Tưởng:** Dịch file JSON sang Code Python và test Sandbox cục bộ.
  ```bash
...
### LLM Pipeline Tuning
- **Mô hình AI Local (Ollama):** Hỗ trợ chạy các model cục bộ (vd: `qwen:qwen3.5:4b`) hoàn toàn tự động. Script sẽ tự bật/tắt server Ollama ngầm, tự biên dịch `Modelfile` để tối ưu hóa ngữ cảnh và tự ép xả (unload) khỏi VRAM khi xong vòng lặp.
