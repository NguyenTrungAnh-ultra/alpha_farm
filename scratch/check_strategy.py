...
def check_strategy():
    ...
    def handle_response(response):
        nonlocal captured_perf
        url = response.url
        if "stages/train/performance" in url:
            try:
                captured_perf = response.json()
                logger.info("Captured train performance data successfully!")
            except Exception as e:
                logger.error(f"Failed to parse performance JSON: {e}")
    ...
    with sync_playwright() as p:
        ...
        # Wait up to 10 seconds for performance data to load
        for _ in range(10):
            if captured_perf is not None:
                break
            page.wait_for_timeout(1000)

        if captured_perf:
            print("\n=== XNOQuant Web backtest results for SMA10 SAR ===")
            print(json.dumps(captured_perf["data"], indent=2))
        else:
            logger.error("Timed out waiting for performance data.")
