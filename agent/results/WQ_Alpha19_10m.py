...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        # (close - delay(close, 7)) + delta(close, 7)
        delay_close_7 = self.op.shift(close, 7)
        delta_close_7 = self.op.diff(close, 7)
        expr1 = (close - delay_close_7) + delta_close_7
        
        # sign(expr1)
        sign_expr1 = self.op.where(expr1 > 0, 1.0, self.op.where(expr1 < 0, -1.0, 0.0))
        
        # rank((1 + sum(returns, 250)))
        sum_ret_250 = self.feat.rolling_sum(returns, 250)
        rank_expr2 = self.feat.rolling_rank(1.0 + sum_ret_250, 20)
        
        alpha_val = (-1.0 * sign_expr1) * (1.0 + rank_expr2)
