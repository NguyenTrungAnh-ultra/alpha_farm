...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        sum_vwap_10 = self.feat.rolling_sum(vwap, 10)
        expr1 = open_ - (sum_vwap_10 / 10.0)
        rank_expr1 = self.feat.rolling_rank(expr1, 20)
        
        # rank(close - vwap)
        rank_expr2 = self.feat.rolling_rank(close - vwap, 20)
        # abs(rank(close - vwap))
        abs_rank_expr2 = self.op.where(rank_expr2 > 0, rank_expr2, -rank_expr2)
        
        alpha_val = rank_expr1 * (-1.0 * abs_rank_expr2)
