...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        ts_max_3 = self.feat.rolling_max(vwap - close, 3)
        rank_max = self.feat.rolling_rank(ts_max_3, 20)
        
        # rank(ts_min((vwap - close), 3))
        ts_min_3 = self.feat.rolling_min(vwap - close, 3)
        rank_min = self.feat.rolling_rank(ts_min_3, 20)
        
        # rank(delta(volume, 3))
        delta_vol_3 = self.op.diff(volume, 3)
        rank_vol = self.feat.rolling_rank(delta_vol_3, 20)
        
        alpha_val = (rank_max + rank_min) * rank_vol
