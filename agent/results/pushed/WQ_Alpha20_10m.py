...
class CustomStrategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        delay_high = self.op.shift(high, 1)
        delay_close = self.op.shift(close, 1)
        delay_low = self.op.shift(low, 1)
        
        rank1 = self.feat.rolling_rank(open_ - delay_high, 20)
        rank2 = self.feat.rolling_rank(open_ - delay_close, 20)
        rank3 = self.feat.rolling_rank(open_ - delay_low, 20)
        
        alpha_val = ((-1.0 * rank1) * rank2) * rank3
