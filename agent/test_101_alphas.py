...
class Alpha{alpha_num}Strategy(SimpleAlgorithm):
    def __algorithm__(self):
        ...
        # Calculate alpha
        try:
            raw_alpha = {formula_expr}
            
            # "Boong-ke" safety: replace Inf/NaN with 0.0
            safe_alpha = self.op.where(self.op.isfinite(raw_alpha), raw_alpha, 0.0)
            
            # Clip explosive signals generated by division by tiny volume
            clipped_alpha = self.op.clip(safe_alpha, -100.0, 100.0)
            
            # Normalize to Z-Score
            alpha_z = self.feat.rolling_zscore(clipped_alpha, window=60)
            
            # Trading logic: Must have volume > 0 to execute
            long_zone = (alpha_z > 1.0) & (vol > 0)
            short_zone = (alpha_z < -1.0) & (vol > 0)
            flat_zone = (~long_zone & ~short_zone) | (vol == 0)
